"""Normalization, exact hashing, and winnowing fingerprints for zlib
version/similarity matching. Shared by build_reference_db.py and match_target.py.

TEMPLATE NOTE: this algorithm is identical across components (FreeRTOS, Mbed TLS) - keep
it that way rather than component-specific. Rename this file to
<component>_fingerprint.py; no other edits should be needed. Kept as its own copy per
component per this repo's layout convention (components/<name>/experiments/ is
self-contained), not shared code across components.
"""
...
def normalize(source: str) -> str:
    ...
    Deliberate consequence: a change confined to a comment (e.g. a license-header SPDX
    line edit - see general/README.md's note on license divergence) normalizes away and
    will exact-match. That's correct for version/provenance matching; a license check
    needs a separate, unnormalized pass if that matters for this component.
    """
