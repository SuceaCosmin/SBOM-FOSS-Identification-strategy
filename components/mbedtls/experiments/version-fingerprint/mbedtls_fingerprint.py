"""Normalization, exact hashing, and winnowing fingerprints for Mbed TLS
version/similarity matching. Shared by build_reference_db.py and match_target.py.

Identical algorithm to the FreeRTOS-Kernel experiment
(components/freertos/experiments/version-fingerprint/freertos_fingerprint.py) - kept as
its own copy per component per this repo's layout convention, not shared code.
"""
...
def normalize(source: str) -> str:
    ...
    Deliberate consequence for Mbed TLS specifically: the SPDX-License-Identifier line
    lives inside each file's header comment, so a vendor that only edits that line (e.g.
    dropping the GPL option - see README.md section 5) normalizes to byte-identical
    content and will exact-match upstream. That's correct for version/provenance
    matching and is why license text needs a separate, unnormalized check.
    """
